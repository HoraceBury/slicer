...
local function curveSliceTouch(e)
	...
	elseif (e.phase == "moved") then
		...
		g.curve = math.dedupePoints( g.curve, 2 )
...
btn.line = display.newImage(btn,"linebtn.png")
btn.curve = display.newImage(btn,"curvebtn.png")
btn.x, btn.y = btn.line.width, btn.line.height
btn.xScale, btn.yScale = 2, 2
btn.xScale, btn.yScale = 2, 2
btn.curve.x = btn.curve.width*2

btn.line:addEventListener("touch",function(e)
	if (e.phase == "ended") then
		Runtime:removeEventListener("touch",curveSliceTouch)
...
btn.curve:addEventListener("touch",function(e)
	if (e.phase == "ended") then
		Runtime:removeEventListener("touch",lineSliceTouch)
		Runtime:addEventListener("touch",curveSliceTouch)
		print("CURVE MODE")
	end
	return true
end)

btn.curve:dispatchEvent{ name="touch", phase="ended" }
